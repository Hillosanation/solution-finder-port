...
impl<'a> RotateReachable<'a> {
    ...
    fn first_check(&mut self, field: &dyn Field, mino: &'static Mino, x: u8, y: u8) -> bool {
...
// same as LockedCandidate, but the first search only searches for rotations
impl Reachable for RotateReachable<'_> {
    fn checks(
        &mut self,
        field: &dyn Field,
        mino: &'static Mino,
    ...
    fn check(
        ...
        self.first_check(field, mino, x, y)
    }
}
...
mod tests {
    ...
    // test cases cloned from locked_reachable
    mod only_90 {
        use super::*;
        ...
        mod with_i {
            ...
            // extra test from RotateReachable
            #[test]
            fn checks_empty() {
                #[rustfmt::skip]
                test_wrapper(
