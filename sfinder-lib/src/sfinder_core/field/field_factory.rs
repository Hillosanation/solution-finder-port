...
pub fn create_field_with_marks(marks: String) -> Box<dyn Field> {
    ...
    match marks.len() / 10 {
        ..=6 => Box::new(create_small_field_with_marks(marks, true)),
        7..=12 => Box::new(create_middle_field_with_marks(marks, true)),
...
fn create_small_field_with_marks(marks: String, is_block: bool) -> SmallField {
    // these conditions are alreadty checked
    ...
    for y in 0..max_y {
        for x in 0..10 {
            match marks[((max_y - y - 1) * 10 + x) as usize] {
                b' ' | b'_' if is_block => field.set_block(x, y),
                _ if !is_block => field.set_block(x, y),
                _ => (),
            }
        }
    }
...
fn create_middle_field_with_marks(marks: String, is_block: bool) -> SmallField {
    todo!("MiddleField")
}
